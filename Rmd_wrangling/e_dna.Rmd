...
pan_edna_metadata <- full_pan_edna %>% 
  ...
  mutate(panama_survey_id = UUIDgenerate(output = "uuid", n = n()),
         visit_id = as.UUID("")) %>% 
  select(!c(geometry))
...
sn_survey <- sn_edna %>% 
  select(site, visit_date, wind, sky, description, c(edna_filter_id:edna_sample_comment)) %>% 
  select(!c(frozen_hrs)) %>% 
  rename(id_edna_sample = edna_filter_id,
         edna_comments = edna_sample_comment, 
         start_time = start_time_edna,
         end_time = end_time_edna) %>% 
  mutate(control = if_else(str_detect(id_edna_sample, "Negative"), 1, 0))
