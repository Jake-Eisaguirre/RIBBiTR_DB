...
unique_cap <- mid_cap %>% 
  left_join(mid_samp_proc, by = c("site" = "location", "date", "bag_id")) %>% 
  select(!c(survey_time.y, species_capture.y, species_capture_other, life_stage_other, sex_other,
            survey_comments)) %>% 
...
dbAppendTable(connection, "penn_cmr", final_cmr)
