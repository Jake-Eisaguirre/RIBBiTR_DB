...
penn1_capture <- plyr::rbind.fill(penn1_capture, june_july_missing_data) %>% 
  mutate(amp_id_1 = if_else(amp_id_1 == "AMPBath00004", "AMPBath00004_5", amp_id_1),
         amp_id_1 = if_else(amp_id_1 == "AMPBath00016", "AMPBath00016_17", amp_id_1),
         amp_id_1 = if_else(amp_id_1 == "AMPBath00018", "AMPBath00018_19", amp_id_1),
         amp_id_1 = if_else(amp_id_1 == "AMPBath00020", "AMPBath00020_21", amp_id_1),
         antibody_id_1 = if_else(antibody_id_1 == "AntiBod00016", "AntiBod00016_17", antibody_id_1),
         antibody_id_1 = if_else(antibody_id_1 == "AntiBod00018", "AntiBod00018_19", antibody_id_1),
         antibody_id_1 = if_else(antibody_id_1 == "AntiBod00020", "AntiBod00020_21", antibody_id_1))
...
brazil_survey <- plyr::rbind.fill(brazil_legacy_survey_table, new_brazil_survey) %>% 
...
survey <- plyr::rbind.fill(brazil_survey, penn_survey_table, sn_survey_table, panama_survey_table, serdp_survey_table) %>% 
  ...
  mutate(survey_id = UUIDgenerate(output = c("uuid")),
         visit_id = as.UUID("")) %>% 
  select(!c(conductivity_uS_cm, qaqc_survey))
...
panama_capture_2022 <- read_csv(here("data", "panama_legacy", "clean_tables", "capture.csv")) %>% 
  ...
  mutate(capture_animal_state = case_when(capture_animal_state == "1" ~ "dead",
         ...
         antibody_id = if_else(antibody_id == "1", swab_id, NULL),
...
capture <- plyr::rbind.fill(panama_capture, brazil_capture, penn_capture, serdp_capture,
  ...
  mutate(capture_id = UUIDgenerate(output = c("uuid"), n = n()),
         survey_id = as.UUID("")) %>% 
  select(!c(amp_id_2:antibody_id_4, sex_other))
