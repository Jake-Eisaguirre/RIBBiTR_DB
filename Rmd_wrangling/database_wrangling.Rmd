...
# survey <- plyr::rbind.fill(sn_survey_table, panama_survey_table, serdp_survey_table, penn_survey_table, brazil_survey)
...
capture <- plyr::rbind.fill(panama_capture, brazil_legacy_capture, penn_capture, serdp_capture,
  ...
  relocate(c(mucosome_id:ampshcl), .after = e_dna) %>% 
  relocate(c(crispr_id:antibody_id_4), .after = ampshcl) %>% 
  relocate(notes_capture, .after = antibody_id_4)
