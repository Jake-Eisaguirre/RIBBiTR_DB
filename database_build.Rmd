...
panama_raw <- read_csv(here("data", "panama_legacy", "panama_data.csv")) %>% 
  ...
  mutate(start_hour = hour(anytime(start_time)),
         end_hour = hour(anytime(end_time)),
    survey_time = if_else(is.na(survey_time), (case_when(start_hour >= 6 & end_hour >= 6 & 
                                                                 end_hour < 19 ~ "day",start_hour >= 19 
                                                               & (end_hour < 6 | end_hour <= 23) |
                                                                 (start_hour < 6 & end_hour < 6)~ "night",
                                                               start_hour >=19 ~"night")), survey_time))
...
panama_ves <- panama_raw %>% 
  ...
  select(site, date, transect, survey_time, start_time, end_time, 
...
panama_aural <- panama_raw %>% 
  ...
  select(site, date, transect, survey_time, 
...
panama_capture <- panama_raw %>% 
  ...
  select(c(site, transect, date, swab_id, species, detection_type, capture_time,
           ...
           notes_amphib2, microhab0, start_time, end_time)) %>% 
...
# ```{r}
# 
# dbExecute(connection, 'create extension if not exists "uuid-ossp"')
# 
# ```
# 
# ## Location: add unique ID and p.keyy
# ```{r}
...
# ```{r}
# dbExecute(connection, 
#           'alter table region add column region_id UUID default (public.uuid_generate_v4());')
# 
# dbExecute(connection, 
#           'alter table region add primary key(region_id);')
# 
# dbExecute(connection, 
#           'alter table region add column location_id uuid;')
# 
# dbExecute(connection, 
#           'update region r
#            set location_id = 
